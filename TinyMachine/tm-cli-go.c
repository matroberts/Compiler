...
/*
   Modification History
      ...
      November 2007   Mike Volz and Eugene Wallingford
         ...
         placing them at locations [1..n] in the DMEM.

      December 2013   Eugene Wallingford
                      University of Northern Iowa
         Replaced uses of gets() with fgets(), which is safer.
         Eliminates run-time warning.

         * Simulates the TM program immediately, rather than
         interacting with the user through the simulator's
         menu interface.

      November 2015   Brian Paterni and Eugene Wallingford
                      University of Northern Iowa
  ...
  { /* RR instructions */
    ...
    /***********************************/
      ...
      { printf("Enter value for IN instruction: ") ;
        ...
        fflush (stdout);
        // gets(in_Line);                           // unsafe
        fgets( in_Line, sizeof(in_Line), stdin );   // safe, with addition
        in_Line[strcspn(in_Line, "\r\n")] = 0;      // of null terminator
...
int main( int argc, char * argv[] )
{
  ...
  dMem[0] = DADDR_SIZE - 1 ;
  backupMem[0] = DADDR_SIZE - 1;

  // --------------------------------------------------------------
  // Modification to pass command-line arguments into the simulator
  // --------------------------------------------------------------
  while (commandLineCounter + 1 < argc)
  {
     if (!strcmp("false", argv[commandLineCounter+1]))
     {
        backupMem[commandLineCounter] = FALSE;
     }
     else if (!strcmp("true", argv[commandLineCounter+1]))
     {
        backupMem[commandLineCounter] = TRUE;
     }
     else
     {
        stringValue = atoi(argv[commandLineCounter+1]);
        if (stringValue == 0 && strcmp("0", argv[commandLineCounter+1]))
        {
           printf("%s is not a valid command line argument\n",
                  argv[commandLineCounter+2]);
           printf("true, false or integer values are valid arguments\n");
           exit(1);
        }
        else
        { 
           backupMem[commandLineCounter] = stringValue;
        }
     }
     commandLineCounter++;     
  }
  
  // test the values in dMem
  //
  // for (commandLineCounter = 1;
  //      commandLineCounter+1 < argc;
  //      commandLineCounter++)
  // {
  //    printf("Memory location %d is %d\n",commandLineCounter,
  //           dMem[commandLineCounter]);
  // }
  ...
  // instead, run the program and exit
  // ---------------------------------
  int stepcnt    = 0;
